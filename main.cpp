#include <cstdio>
#include <cstdlib>
#include <pthread.h>
#include <unistd.h>
#include "math.h"

#define PHILO 5
#define DELAY 30000
#define FOOD 50

pthread_mutex_t forks[PHILO];
pthread_t phils[PHILO];
void *philosopher (void *num);
int food_on_table ();
void get_fork (int, int, char *);
void down_forks (int, int);
pthread_mutex_t foodlock;

typedef struct Philosopher{
    int phil_num;
}philo;

int sleep_seconds = 0;

int main (int argn, char **argv){
  int i;

  if (argn == 2)
    sleep_seconds = atoi (argv[1]);

  pthread_mutex_init (&foodlock, nullptr);
  for (i = 0; i < PHILO; i++)
    pthread_mutex_init (&forks[i], nullptr);
  for (i = 0; i < PHILO; i++) {
      auto* philosophers = (philo*) malloc(sizeof(philo));
      philosophers->phil_num=i;
      pthread_create(&phils[i], nullptr, philosopher, (void *)philosophers);
  }
  for (i = 0; i < PHILO; i++)
    pthread_join (phils[i], nullptr);
  return 0;
}



void * philosopher (void *num){
  int id;
  int left_fork, right_fork, f;

  id = ((philo*)num)->phil_num;
  free(num);
  printf ("Philosopher %d sitting down to dinner.\n", id);
  right_fork = id;
  left_fork = id + 1;

  /* Wrap around the forks. */
    if (left_fork == PHILO)
        left_fork = 0;

    f = food_on_table();
    while (f) {

        /* Thanks to philosophers #1 who would like to
         * take a nap before picking up the forks, the other
         * philosophers may be able to eat their dishes and
         * not deadlock.
         */
        if (id == 1)
            sleep (sleep_seconds);

        printf ("Philosopher %d: get dish %d.\n", id, f);
        if(id!=PHILO) {
            get_fork(id, right_fork, "right");
            get_fork(id, left_fork, "left ");
        }else{
            get_fork(id, left_fork, "left ");
            get_fork(id, right_fork, "right");
        }

        printf ("Philosopher %d: eating.\n", id);
        usleep (DELAY * (FOOD - f + 1));
        down_forks(left_fork, right_fork);
        f = food_on_table();
    }
    printf ("Philosopher %d is done eating.\n", id);
    return (NULL);
}

int food_on_table (){
  static int food = FOOD;
  int myfood;
  pthread_mutex_lock (&foodlock);
  if (food > 0) {
    food--;
  }
  myfood = food;
  pthread_mutex_unlock (&foodlock);
  return myfood;
}

void
get_fork (int phil, int fork, char *hand){
    pthread_mutex_lock (&forks[fork]);
    printf ("Philosopher %d: got %s fork %d\n", phil, hand, fork);
}

void
down_forks (int f1, int f2){
    pthread_mutex_unlock (&forks[f1]);
    pthread_mutex_unlock (&forks[f2]);
}
